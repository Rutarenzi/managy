...
const { assetItemReference } = require("../../../models/referenceData/assetItemReference ");
const { assetItemReferenceSchema } = require("./schema");

module.exports.create = async (req, res) => {
    try {
        const { value, error } = assetItemReferenceSchema.validate(req.body)
        if (error) throw new Error(error.message)

        const exists = await assetItemReference.findOne({ name: value.name })
        if (exists) throw new Error("An Asset Functional Location with that name already exists")
        const newAssetMl = await assetItemReference.create({
            ...value,
...
module.exports.getAll = async (req, res) => {
    try {
        const allAssetML = await assetItemReference.find().populate('assetCategory');
        console.log(allAssetML)
...
module.exports.getById = async (req, res) => {
    try {
        const assetML = await assetItemReference.findById(req.params.id)
...
module.exports.updateById = async (req, res) => {
    try {
        const assetMLExists = await assetItemReference.findById(req.params.id)
        if (!assetMLExists) throw new Error("Not found")

        const { value, error } = assetItemReferenceSchema.validate(req.body)
        if (error) throw new Error(error.message)

        const updatedAssetML = await assetItemReference.findByIdAndUpdate(req.params.id, {
            value
        })
...
module.exports.deletebyId = async (req, res) => {
    try {
        const deleted = await assetItemReference.findByIdAndDelete(req.params.id)
